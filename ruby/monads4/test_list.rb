...
def test3
  ...
  puts "-"*80
  # nested
  List.from(BLOGS).then { |blog|
    List.from(blog.categories).then { |category|
      List.from(category.posts).then { |post|
        List.from(post.comments).then { |comment|
...
words = List.from(BLOGS)
.then { |blog| blog.categories }
.then { |category| category.posts }
.then { |post| post.comments }
.then { |comment| comment.split(/\s+/) }
.then { |word| word }
blogs = List.from(BLOGS)
