...
def div(x, y)
  return Maybe.from(x).then{ |v| v*2 }.then{ |v| v/y }.then{ |v| v + 1 }
end

def division(x, y)
  div(x, y).then{ |v| return v }.catch{ |err| puts "ERROR: #{err}" }
  return nil
end

def division_raise(x, y)
  div(x, y).then{ |v| return v }.raise_on_error
  return nil
end

def division_nan(x, y)
  div(x, y).catch{ 'NAN' }.then{ |v| return v }.catch{ |err| puts "ERROR: #{err}" }
  return nil
end

puts division 10,3
puts division 10,0
puts division_nan 10,3
puts division_nan 10,0

puts division_raise 10,3
puts division_raise 10,0
