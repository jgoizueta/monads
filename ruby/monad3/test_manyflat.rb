...
def test3
  ...
  puts "-"*80
  # nested
  ManyFlat.from(BLOGS).then { |blog|
    ManyFlat.from(blog.categories).then { |category|
      ManyFlat.from(category.posts).then { |post|
        ManyFlat.from(post.comments).then { |comment|
          puts "#{blog.id}/#{category.id}/#{post.id}/#{comment}"
          ManyFlat.from(comment)
        }
      }
    }
  }
...
words = ManyFlat.from(BLOGS)
.then { |blog| blog.categories }
.then { |category| category.posts }
.then { |post| post.comments }
.then { |comment| comment.split(/\s+/) }
.then { |word| word }
blogs = ManyFlat.from(BLOGS)
